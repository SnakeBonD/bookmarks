const CACHE='bookmarks-v0.9-shell';
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('bookmarks-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
