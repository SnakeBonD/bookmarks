...
for(const s of ['Bookmarks v0.7','searchInput','bookmarkDialog'])if(!html.includes(s))throw new Error('Missing HTML marker '+s);
for(const s of ['localStorage','AI Tools Hub','renderContent','exportBtn','duplicateGroups','templates','quickNote','initCloud','cloudPush','cloudPull','suggestMetadata','checkCurrentPageLinks','visitCount','importBrowserHtml','exportBrowserHtml','wirePageReorder','serviceWorker','deferredInstallPrompt','updateNetworkStatus','widgetConfig','calendarWidgetHtml','openWidgetSettings'])if(!js.includes(s))throw new Error('Missing JS marker '+s);
