...
function renderWidgets(){const page=currentPage();const count=state.bookmarks.filter(b=>b.pageId===page?.id).length;const pins=state.bookmarks.filter(b=>b.pageId===page?.id&&b.pinned).length;els.widgets.innerHTML=`<div class="widget"><div class="widget-title">Heure</div><div id="clockWidget" class="widget-value"></div></div><div class="widget"><div class="widget-title">Page</div><div class="widget-value">${esc(page?.name||'')}</div><div class="tool-note">${count} favoris · ${pins} épinglés</div></div><div class="widget"><div class="widget-title">Note rapide</div><textarea id="quickNote" class="widget-note" placeholder="Une note temporaire…">${esc(state.settings.note||'')}</textarea></div>`;tickClock();$('#quickNote').oninput=e=>{state.settings.note=e.target.value;save()}}
