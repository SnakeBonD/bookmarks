...
function wireCards(){$('[data-favicon]').forEach(img=>img.addEventListener('error',()=>{img.style.display='none';img.parentElement.textContent='🔗'}));$('[data-edit]').forEach(x=>x.onclick=e=>{e.preventDefault();openBookmark(x.dataset.edit)});$$('[data-visit]').forEach(x=>x.addEventListener('click',()=>{const b=state.bookmarks.find(v=>v.id===x.dataset.visit);if(b){b.visitCount=(b.visitCount||0)+1;b.lastUsedAt=new Date().toISOString();save();renderWidgets()}}));let dragged=null;$$('.bookmark').forEach(el=>{el.addEventListener('dragstart',()=>{dragged=el.dataset.id;el.classList.add('dragging')});el.addEventListener('dragend',()=>el.classList.remove('dragging'));el.addEventListener('dragover',e=>e.preventDefault());el.addEventListener('drop',e=>{e.preventDefault();const target=e.currentTarget.dataset.id;if(!dragged||dragged===target)return;const a=state.bookmarks.findIndex(b=>b.id===dragged),t=state.bookmarks.findIndex(b=>b.id===target);const[item]=state.bookmarks.splice(a,1);state.bookmarks.splice(t,0,item);render()})})}
